...
int main(int argc, char *argv[])
{
  ...
  for(i = 0; i < 16; ++i)
  {
    *(uint32_t *)(cfg + 4 + i * 4) = (uint32_t)floor(modf((1.0 + 1.0e-6 * corr) * freq[i] / 77.76, &integral) * 0xffffffff + 0.5);
  }
  ...
  *rst |= 1;
  ...
  offset = 0;
  buffer = malloc(240000 * 8 * 16);
  memset(buffer, 0, 240000 * 8 * 16);
